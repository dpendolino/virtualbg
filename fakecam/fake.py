...
def get_mask(frame, bodypix_url='http://bodypix:9000'):
    ...
    r = requests.post(url=bodypix_url,
                      data=data.tobytes(),
...
def hologram_effect(img):
    ...
    # add some ghosting
    holo_blur = cv2.addWeighted(holo, 0.2, shift_image(holo.copy(), 5, 5), 0.8,
                                0)
    holo_blur = cv2.addWeighted(holo_blur, 0.4,
                                shift_image(holo.copy(), -5, -5), 0.6, 0)
...
def get_frame(cap, background_scaled):
    ...
    for c in range(frame.shape[2]):
        frame[:, :,
              c] = frame[:, :, c] * mask + background_scaled[:, :,
                                                             c] * inv_mask
    return frame
